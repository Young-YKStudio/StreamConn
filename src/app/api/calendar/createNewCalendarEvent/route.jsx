...
export async function POST(req) {
  ...
  const {title, start, end, calendarOwner} = receivedData
  ...
  const createdCalendarEvent = await Calendar.create({
    title: title,
    start: start,
    end: end,
  ...
  if(!createdCalendarEvent) {
    return NextResponse.json(
      {message: 'Error at creating calendar event. Please try again.'},
      {status: 500}
    )
  }
