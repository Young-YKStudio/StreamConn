...
const SideBar = () => {
  ...
  const styleDistributor = (section) => {
    if(section) {
      return ''
    }
    if(!section) {
  ...
  return (
    <nav className="bg-slate-800 h-screen pt-20 w-48 px-4">
      ...
      <div className='my-2 flex flex-row justify-between'>
        ...
        <button onClick={collapseHandler}>{isSectionCollapsed ? <BiArrowToLeft className='w-5 h-5'/> : <BiArrowToRight className='w-5 h-5'/>}</button>
      </div>
