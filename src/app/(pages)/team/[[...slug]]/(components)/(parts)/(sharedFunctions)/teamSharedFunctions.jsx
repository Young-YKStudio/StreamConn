...
export const tabButtonStyles = (currentSection, team) => {
...
export const channelTypeButtonStyles = (currentTeamType, selectedTeamType) => {
  if(currentTeamType === selectedTeamType) {
...
export const userValidator = (currentUser, teamOwner) => {
  if(currentUser._id === teamOwner._id) {
    return true
  }
...
export const teamNameValidator = (teamName) => {
  const specialCharacters = `/[!@#$%^&*()+\-=\[\]{};':"\\|,.<>\/?]+/ `
  const notValidated = specialCharacters.split('').some(char => teamName.includes(char)) 
  ...
  if(teamName.length < 2 || teamName.length > 16) {
    toast.error('Team name must be between 2 - 16 characters long')
    return true
  }
...
export const profileButtonDistributor = (loggedUser, team, setModalOn, addFollowFunction, addSubscriptionFunction, isFollowedButtonHovered, setHoverOnFollows, setHoverOffFollows, unfollowFunction) => {
  if(!loggedUser) {    
    return (
  ...
  if(loggedUser._id == team.teamOwner._id) {
    return (
      <div className="flex items-end h-full gap-2 pb-4 text-sm">
  ...
  let followedTeam = loggedUser.teams.find((foundTeam) => foundTeam.teamOwner._id == team.teamOwner._id)
